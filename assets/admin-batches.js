(() => {
  ...
  async function render() {
    ...
    if (!body || document.querySelector('#adminApp')?.style.display === 'none') return;
    try {
      ...
      document.querySelectorAll('.batch-confirm').forEach(btn => {
        btn.onclick = async () => {
          const ok = confirm('確認後，此週既有收益分配將被鎖定且不能重新計算。確定要確認結算嗎？');
          if (!ok) return;
          try {
  ...
  // Demo mode shows immediately; live mode will populate after successful login.
  setTimeout(originalBootTrigger, 50);
